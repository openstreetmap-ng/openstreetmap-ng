...
class ElementRepository:
    ...
    @staticmethod
    async def get_many_latest_members_by_element(
        parent: Element,
        *,
        recurse_ways: bool = False,
        limit: int | None = FIND_LIMIT,
        ...
        """
        Get elements that are referenced by the given element.
        ...
        point_in_time = utcnow()
        recurse_way_refs = (
            tuple(member for member in parent.members if member.type == ElementType.way) if recurse_ways else ()
        )
            ...
            stmt = select(Element).where(
                ...
                or_(
                    and_(
                        ...
                        Element.typed_id == member.typed_id,
                    )
                    ...
                    select(Element).where(
                        ...
                        Element.typed_id.in_(
                            ...
                            .where(
                                ...
                                Element.superseded_at == null() | (Element.superseded_at > point_in_time),
                                Element.type == ElementType.way,
                                Element.typed_id.in_(member.typed_id for member in recurse_way_refs),
                            )
                            .subquery()
                        ),
                    )
