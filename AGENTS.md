...
- Router integration: for map/index pages, define `IndexRouteDef` routes and register them via `configureRouter([...])` (`app/views/index/router.ts`). Render the active route inside `IndexRouterOutlet` (`app/views/index/router-outlet.tsx`) mounted into `#ActionSidebar` (`app/views/lib/map/main-map.tsx`). Treat the URL as the source of truth: routes receive a reactive `ctx` signal with parsed `pathParams` + `queryParams` so components can avoid `window.location.search` and ad‑hoc parsing boilerplate.
