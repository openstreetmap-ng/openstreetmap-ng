...
- TypeScript: `tsconfig.json` has `"strict": true` but `"strictNullChecks": false`. This end‑to‑end app prefers fail‑fast crashes over silently continuing with unexpected nullish states. Don't re‑enable null checks locally. Avoid defensive optional chaining (`?.`) or fallback operators (`??`) on values guaranteed by template or control flow—these hide bugs instead of surfacing them.
