...
static PyTypeObject CDATAType = {
    PyVarObject_HEAD_INIT(NULL, 0)
        .tp_name = "optimized.xml_unparse.CDATA",
...
// If both fields are NULL, an error has occurred.
typedef struct
{
    const char *str;
    PyObject *obj;
...
static toStringResult to_string(PyObject *value)
{
    if (value == Py_True)
...
static bool unparse_element(xmlDocPtr doc, xmlNodePtr parent, const char *key, PyObject *value, bool is_root);
...
static bool unparse_scalar(xmlDocPtr doc, xmlNodePtr parent, const char *key, PyObject *value)
{
    ...
    toStringResult result = to_string(value);
    int add_result;
...
static bool unparse_item(xmlDocPtr doc, xmlNodePtr element, const char *key, PyObject *value)
{
    if (key[0] == '@')
    { // Is attribute
        toStringResult result = to_string(value);
...
static bool unparse_element(xmlDocPtr doc, xmlNodePtr parent, const char *key, PyObject *value, bool is_root)
{
    ...
    { // Encode sequence of ...
        const bool is_list = PyList_CheckExact(value);
        const Py_ssize_t size = is_list ? PyList_GET_SIZE(value) : PyTuple_GET_SIZE(value);
        xmlNodePtr tuples_element = NULL;
...
static PyObject *xml_unparse(const PyObject *, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (UNLIKELY(!unparse_element(doc, dummy, PyUnicode_AsUTF8(key), value, true)))
    {
        ...
        if (error)
            PyErr_Format(PyExc_ValueError, "Error unparsing XML: %s", error->message ? error->message : "Unknown error");
        xmlResetLastError();
        xmlFreeDoc(doc);
        xmlFreeNode(dummy);
        return NULL;
...
static PyMethodDef methods[] = {
    {"xml_unparse", _PyCFunction_CAST(xml_unparse), METH_FASTCALL, NULL},
    {NULL, NULL, 0, NULL},
};
...
PyMODINIT_FUNC PyInit_xml_unparse(void)
{
    PyDateTime_IMPORT;
