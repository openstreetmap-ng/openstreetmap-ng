...
class OptimisticDiffApply:
    async def apply(self, prepare: OptimisticDiffPrepare) -> dict[ElementRef, Sequence[Element]]:
        ...
        async with db_commit() as session, create_task_group() as tg:
            ...
            now = utcnow()
            tg.start_soon(_update_changesets, prepare.changeset_state, now, session)
            tg.start_soon(_update_elements, prepare.elements, now, session)
...
async def _update_changesets(changeset_state: dict[int, Changeset], now: datetime, session: AsyncSession) -> None:
    """
...
async def _update_elements(elements: Sequence[Element], now: datetime, session: AsyncSession) -> None:
    """
    ...
    await _update_elements_db(current_sequence_id, update_type_ids, elements, insert_members, session)
