...
import { renderObjects } from "@map/render-objects"
import {
  batch,
  type ReadonlySignal,
  useComputed,
...
import { pathParam, queryParam } from "@utils/codecs"
...
import type { Bounds, OSMChangeset } from "@utils/types"
...
const LAYER_ID = "changeset-history" as LayerId
const LAYER_ID_BORDERS = "changeset-history-borders" as LayerId
...
const ChangesetHistorySidebar = ({
  ...
  const fetchChangesets = async (options?: { fromMapMove?: boolean }) => {
    ...
    try {
      ...
      console.error("ChangesetHistory: Failed to fetch", error)
    } finally {
      token.done()
...
export const ChangesetHistoryRoute = defineRoute({
  id: "changeset-history",
  ...
  Component: ChangesetHistorySidebar,
})
