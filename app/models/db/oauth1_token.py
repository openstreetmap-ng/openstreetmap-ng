...
class OAuth1Token(Base.Snowflake, CreatedAtMixin):
    ...
    application_id: Mapped[int] = mapped_column(ForeignKey(OAuth1Application.id, ondelete='CASCADE'), nullable=False)
    application: Mapped[OAuth1Application] = relationship(lazy='raise', innerjoin=True)
