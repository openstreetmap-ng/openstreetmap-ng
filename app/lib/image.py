...
class Image:
    ...
    async def process_proxy_image(data: bytes) -> bytes:
        ...
        No aspect ratio constraints (free ratio). NSFW detection included.
        """
        # Use 0 for ratios to skip cropping, pass max_dimension for simpler resizing
        return await _normalize_image(
            data,
            min_ratio=0,
            max_ratio=0,
            max_megapixels=IMAGE_PROXY_MAX_DIMENSION * IMAGE_PROXY_MAX_DIMENSION,
    ...
    async def create_thumbnail(data: bytes) -> bytes:
        """
        Create a tiny thumbnail from image data.

        Used for Low Quality Image Placeholder (LQIP) pattern.
        Simple resize to small dimension with low quality.
        """
        ...
        max_dim = IMAGE_PROXY_THUMBNAIL_MAX_DIMENSION
        if img_width > img_height:
            new_width: cython.int = max_dim
            new_height: cython.int = max(1, int(img_height * (max_dim / img_width)))
        else:
            new_height = max_dim
            new_width = max(1, int(img_width * (max_dim / img_height)))

        img = cv2.resize(img, (new_width, new_height), interpolation=cv2.INTER_AREA)

        # Encode with low quality for minimal size
        _, img_encoded = await loop.run_in_executor(
